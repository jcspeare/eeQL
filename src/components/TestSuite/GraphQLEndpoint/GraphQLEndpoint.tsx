...
//endpoint test
  const queryValidity = () => {
    const newTest = `describe('${test.expectedRes}', () => {
      it('${test.expectedRes}', () => {
        post(${test.inputData});
      tester.test(${true}, ${test.outputData})
    })`;
  ...
  return (
    <div>
      ...
      <div className='rest-endpoint'>
        ...
        <br />
        <br />
        <Input placeholder='Describe Your Test' defaultValue={test.expectedRes || ''}fullWidth={true} id="expectedRes" type='text' onChange={inputHandler} />
        ...
        <br></br>
        <button onClick={() => queryValidity()}>Test button</button>
      </div>
    </div>
  );
};
